...
class Channel extends Common
{
    ...
    public function add_channel()
    {
        if(request()->isAjax()){
            ...
            if($status){
                cache('channel', null);
                $this->result('insert channel data is success',200,'栏目添加成功，正在刷新页面');
            }else{
    ...
    public function ed_channel(){
        if(request()->isAjax()){
            ...
            if($status){
                cache('channel',null);
                $this->result('update is ok',200,'更新成功，正在刷新页面，请稍等...');
            }else{
    ...
    public function del_channel()
    {
        ...
        if (isset($id)) {
            ...
            if ($status) {
                ...
                $this->result('delete channel is success', 200, '删除成功，正在为您新页面，请稍等...');
            } else {
