...
$(document).ready(function(){
    ...
    $(document).on('click','.order-status>.weui-navbar__item',function(){
        $(this).addClass('weui-bar__item_on').siblings('.order-status>.weui-navbar__item').removeClass('weui-bar__item_on');
        var orderStatus = $(this).data('status');
        $.get("/mshop/api/order_list.html",{status:orderStatus},function(res){
            if(res.code == 200){
...
function addShopCart(f){
    ...
    $.post(url,formDataSerialize,function(res){
        if(res.code == 200){
            showAlert(200,'加入购物车成功,请点击结算');
        }else if(res.code == 403){
            showAlert(res.code,res.msg);
            setTimeout("window.location.href = '/mshop/user/index'",2000);
        }else {
            showAlert(res.code,res.msg);
        }
    })
...
function toBuyGoods(f){
    ...
    $.post('/mshop/pay/create_order',data,function(call){
        if(call.code == 200){
            ...
            window.location.href='/mshop/user/index';
        } else {
